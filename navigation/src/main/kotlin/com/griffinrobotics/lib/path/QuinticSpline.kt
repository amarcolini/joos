...
class QuinticSpline(
    ...
    private val x: QuinticPolynomial =
        QuinticPolynomial(start.x, start.dx, start.d2x, end.x, end.dx, end.d2x)
    ...
    override fun internalDeriv(t: Double): Vector2d {
        return Vector2d(x.deriv(t), y.deriv(t))
    }

    override fun internalSecondDeriv(t: Double): Vector2d {
        return Vector2d(x.secondDeriv(t), y.secondDeriv(t))
    }

    override fun internalThirdDeriv(t: Double): Vector2d {
        return Vector2d(x.thirdDeriv(t), y.thirdDeriv(t))
    }
